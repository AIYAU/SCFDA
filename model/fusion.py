...
class TransformerFusionBlock(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h=4, block_exp=4, n_layer=1, embd_pdrop=0.1, attn_pdrop=0.1, resid_pdrop=0.1):
        ...
        self.crosstransformer = nn.Sequential \
            (*[CrossTransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop) for layer in range(n_layer)])
        # Concat
    ...
    def forward(self, x):
        ...
        #     feature = transforms.ToPILImage()(block[i][:3].squeeze()) # 我自己加了:3
...
if __name__ == '__main__':
    # 确保 d_model 与输入张量的通道数一致
    d_model = 100  # 输入张量的通道数
    vert_anchors = 7  # 自适应池化的输出高度
    horz_anchors = 7  # 自适应池化的输出宽度
    # 初始化 TransformerFusionBlock
    tfusion = TransformerFusionBlock(d_model=d_model, vert_anchors=vert_anchors, horz_anchors=horz_anchors)
    # 创建输入张量，形状为 (batch_size, channels, height, width)
    batch_size = 16
    input_height = 7
    input_width = 7
    input_tensor = torch.rand(batch_size, d_model, input_height, input_width)
    # 运行模型
    print(input_tensor.shape)
    output = tfusion((input_tensor, input_tensor))
    # 打印输出张量的形状
    print(output.shape)
