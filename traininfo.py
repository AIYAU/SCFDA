...
from tllib.modules import GaussianKernel
from torch.autograd import Variable
from torch.utils.tensorboard import SummaryWriter

# from RAGModule.utils.sim import  batch_top_n_similarities
from model.mapping import Mapping
# from model.encoder import Encoder, FrequencyFeatureExtractor
from model.Tdencoder import Encoder
from utils.dataloader import get_HBKC_data_loader, Task, get_target_dataset, tagetSSLDataset, get_target_dataset_houston
from utils import utils, loss_function, data_augment

parser = argparse.ArgumentParser(description="Few Shot Visual Recognition")
parser.add_argument('--config', type=str, default=os.path.join('./config', 'Indian_pines.py'))  # 加载配置文件
...
########################## add rag module##################
# loaded_vectors = np.load('./RAGModule/new_vectors.npy', allow_pickle=True) # 索引检索增强

# src_vectors, _, _ = batch_top_n_similarities(torch.from_numpy(semantic_mapping_src),torch.from_numpy(loaded_vectors), 3)
# semantic_mapping_src = 0.5 * semantic_mapping_src + 0.5 * np.array(src_vectors)
# tar_vectors, _, _ = batch_top_n_similarities(torch.from_numpy(semantic_mapping_tar),torch.from_numpy(loaded_vectors), 3)
...
coral_loss = CorrelationAlignmentLoss().to(GPU)
# experimental result index 实验结果指标
nDataSet = 10# 取10个结果
...
seeds = [1236, 1237, 1226, 1227, 1211, 1212, 1216, 1221, 1222, 1223]
# seeds = [1223, 1237, 1226, 1227, 1211, 1212, 1216, 1240, 1222, 1223]
# log setting # 记录日志
...
for iDataSet in range(nDataSet):
    ...
    mapping_src = Mapping(SRC_INPUT_DIMENSION, N_DIMENSION).to(GPU) # 源域特征映射
    mapping_tar = Mapping(TAR_INPUT_DIMENSION, N_DIMENSION).to(GPU) #目标域特征映射
    encoder = Encoder(n_dimension=N_DIMENSION, patch_size=patch_size, sub_patch_size=3,emb_size=emb_size, dropout=0.3).to(GPU)
    ...
    for episode in range(EPISODE):
        ...
        support_features_src, semantic_feature_src,freq_feature_src = encoder(mapping_src(support_src.to(GPU)),semantic_feature=semantic_support_src.to(GPU),s_or_q="support")  # (9, 160)
        # 查询集没标签直接提取特征
        query_features_src = encoder(mapping_src(query_src.to(GPU)))
        # 把目标域数据进行特征提取，同时把语义信息穿进去
        support_features_tar, semantic_feature_tar,freq_feature_tar = encoder(mapping_tar(support_tar.to(GPU)),semantic_feature=semantic_support_tar.to(GPU),s_or_q="support")  # (9, 160)
        # 目标域查询集
        ...
        # scl_loss_tar = infoNCE_Loss(semantic_feature_src,freq_feature_src) + infoNCE_Loss(semantic_feature_tar,freq_feature_tar)
        ##KL
        text_align_loss = infoNCE_Loss(semantic_feature_src, support_features_src) + infoNCE_Loss(semantic_feature_tar,support_features_tar)
        scl_loss_tar = coral_loss(semantic_feature_src,freq_feature_src) + coral_loss(semantic_feature_tar,freq_feature_tar)
        # scl_loss_tar = torch.tensor(0)
        # text_align_loss = jan_loss((semantic_feature_src, support_features_src), (semantic_feature_tar,support_features_tar))
        # scl_loss_tar = jan_loss((semantic_feature_src,freq_feature_src), (semantic_feature_tar,freq_feature_tar))
        ...
        if (episode + 1) % 500 == 0 or episode == 0:
            with torch.no_grad():
                ...
                train_features, _ ,_= encoder(mapping_tar(Variable(train_datas).to(GPU)), semantic_feature=semantic_support.to(GPU), s_or_q="support")
                ...
                train_features = torch.where(torch.isnan(train_features) | torch.isinf(train_features), torch.tensor(0.0, device=train_features.device), train_features)
                ...
                for test_datas, test_labels in test_loader:
                    ...
                    test_features = torch.where(torch.isnan(test_features) | torch.isinf(test_features), torch.tensor(0.0, device=train_features.device), test_features)
